...
function NewCategorySheet({
  ...
  onClose: () => void;
  onDelete?: () => void;
  onSave: (category: IncomeCategory, color: string) => void;
  visible: boolean;
}) {
        ...
        <View pointerEvents="box-none" style={styles.categorySheetContainer}>
          ...
          >
            <View style={styles.categorySheetHeader}>
              <Text style={styles.categorySheetTitle}>Income Category</Text>
...
export default function IncomeCategoriesScreen() {
  ...
  const {
    incomeCategories: storeCategories,
    categoryColors,
    addCategory,
    deleteCategory,
    updateCategory,
  ...
  const handleDelete = useCallback((name: string) => {
    deleteCategory("income", name);
    setLocalCategories((prev) => prev.filter((c) => c.name !== name));
  }, [deleteCategory]);

  const handleAddCategory = useCallback((category: IncomeCategory, color: string) => {
    addCategory("income", category, color);
    setLocalCategories((prev) => [...prev, category]);
  }, [addCategory]);

  const handleUpdateCategory = useCallback((newCategory: IncomeCategory, color: string) => {
    const oldName = editingCategory?.category.name;
    if (!oldName) return;
    updateCategory("income", oldName, newCategory, color);
  ...
  const handleDeleteEditingCategory = useCallback(() => {
    ...
    deleteCategory("income", editingCategory.category.name);
  ...
  const handleDragEnd = useCallback((dy: number) => {
    ...
    setLocalCategories((prev) => {
      ...
      reorderCategories("income", from, to);
      const next = [...prev];
  ...
  return (
    <View style={styles.root}>
      ...
      >
        {/* Header */}
        <View style={styles.categorySheetHeader}>
          <Text style={styles.categorySheetTitle}>Income Categories</Text>
          <View style={styles.categorySheetHeaderActions}>
            <Pressable
